...
func main() {
	...
	for i := 0; i < flag.NArg()-1; i++ {
		var samples = make([]sox.Sample, MAX_SAMPLES)
		...
		// Continue here!
		for number_read := input.Read(samples, 1); number_read != 0; number_read = input.Read(samples, 1) {
			check(output.Write(samples, 1) == number_read, "write")
		}
		input.Release()
	}
	output.Release()
